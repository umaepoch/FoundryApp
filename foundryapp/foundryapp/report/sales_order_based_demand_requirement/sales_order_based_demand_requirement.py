...
def execute(filters=None):
	...
	wfw = get_total_weight_for_week(show)
	rpt = get_report(show)
...
def get_report(sdi):
	...
	if sdi == 1:
		r_data = frappe.db.sql("""select date_sub(date(tsi.delivery_date), interval dayofweek(tsi.delivery_date)-1 day) as delivery_date,
							...
							order by delivery_date, tbi.item_code""", as_dict=1)

	if sdi is None:
		r_data = frappe.db.sql("""select date_sub(date(tsi.delivery_date), interval dayofweek(tsi.delivery_date)-7 day) as delivery_date,
							...
							order by delivery_date, tsi.item_code""", as_dict = 1)
	return r_data

# fetches the sum of weights for a week.
def get_total_weight_for_week(sdi):
	wfw_data =[]

	if sdi == 1:
		wfw_data = frappe.db.sql("""select twfw.delivery_date, sum(twfw.total_weight) as total_weight_for_week
								from (
								select date_sub(date(tsi.delivery_date), interval dayofweek(tsi.delivery_date)-1 day) as delivery_date, tbi.item_code,
								...
								group by twfw.delivery_date""", as_dict=1)

	if sdi is None:
		wfw_data = frappe.db.sql("""select twfw.delivery_date, sum(twfw.total_weight) as total_weight_for_week
								from (
								select date_sub(date(tsi.delivery_date), interval dayofweek(tsi.delivery_date)-7 day) as delivery_date, tsi.item_code,
								sum(tsi.qty) as sum_quantiy, ti.weight_per_unit, ((sum(tsi.qty)*ti.weight_per_unit)/1000) as total_weight
								from `tabSales Order Item` as tsi
								join `tabSales Order` as tso on tso.name = tsi.parent
								join `tabItem` as ti on ti.item_code = tsi.item_code
